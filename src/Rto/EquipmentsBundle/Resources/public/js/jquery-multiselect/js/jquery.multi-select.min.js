...
undefined?that.$selectableUl.children().length:index;selectableLi.insertAt(index,that.$selectableUl);selectedLi.insertAt(index,that.$selectionUl)}},"addOption":function(options){var that=this;if(options.value)options=[options];$.each(options,function(index,option){if(option.value&&that.$element.find("option[value='"+option.value+"']").length===0){var $option=$('<option value="'+option.value+'">'+option.text+"</option>"),index=parseInt(typeof option.index==="undefined"?that.$element.children().length:
option.index),$container=option.nested==undefined?that.$element:$("optgroup[label='"+option.nested+"']");$option.insertAt(index,$container);that.generateLisFromOption($option.get(0),index,option.nested)}})},"escapeHTML":function(text){return $("<div>").text(text).html()},"activeKeyboard":function($list){var that=this;$list.on("focus",function(){$(this).addClass("ms-focus")}).on("blur",function(){$(this).removeClass("ms-focus")}).on("keydown",function(e){switch(e.which){case 40:case 38:e.preventDefault();
e.stopPropagation();that.moveHighlight($(this),e.which===38?-1:1);return;case 37:case 39:e.preventDefault();e.stopPropagation();that.switchList($list);return;case 9:if(that.$element.is("[tabindex]")){e.preventDefault();var tabindex=parseInt(that.$element.attr("tabindex"),10);tabindex=e.shiftKey?tabindex-1:tabindex+1;$('[tabindex="'+tabindex+'"]').focus();return}else if(e.shiftKey)that.$element.trigger("focus")}if($.inArray(e.which,that.options.keySelect)>-1){e.preventDefault();e.stopPropagation();
that.selectHighlighted($list);return}})},"moveHighlight":function($list,direction){var $elems=$list.find(this.elemsSelector),$currElem=$elems.filter(".ms-hover"),$nextElem=null,elemHeight=$elems.first().outerHeight(),containerHeight=$list.height(),containerSelector="#"+this.$container.prop("id");$elems.removeClass("ms-hover");if(direction===1){$nextElem=$currElem.nextAll(this.elemsSelector).first();if($nextElem.length===0){var $optgroupUl=$currElem.parent();if($optgroupUl.hasClass("ms-optgroup")){var $optgroupLi=
$optgroupUl.parent(),$nextOptgroupLi=$optgroupLi.next(":visible");if($nextOptgroupLi.length>0)$nextElem=$nextOptgroupLi.find(this.elemsSelector).first();else $nextElem=$elems.first()}else $nextElem=$elems.first()}}else if(direction===-1){$nextElem=$currElem.prevAll(this.elemsSelector).first();if($nextElem.length===0){var $optgroupUl=$currElem.parent();if($optgroupUl.hasClass("ms-optgroup")){var $optgroupLi=$optgroupUl.parent(),$prevOptgroupLi=$optgroupLi.prev(":visible");if($prevOptgroupLi.length>
0)$nextElem=$prevOptgroupLi.find(this.elemsSelector).last();else $nextElem=$elems.last()}else $nextElem=$elems.last()}}if($nextElem.length>0){$nextElem.addClass("ms-hover");var scrollTo=$list.scrollTop()+$nextElem.position().top-containerHeight/2+elemHeight/2;$list.scrollTop(scrollTo)}},"selectHighlighted":function($list){var $elems=$list.find(this.elemsSelector),$highlightedElem=$elems.filter(".ms-hover").first();if($highlightedElem.length>0){if($list.parent().hasClass("ms-selectable"))this.select($highlightedElem.data("ms-value"));
else this.deselect($highlightedElem.data("ms-value"));$elems.removeClass("ms-hover")}},"switchList":function($list){$list.blur();this.$container.find(this.elemsSelector).removeClass("ms-hover");if($list.parent().hasClass("ms-selectable"))this.$selectionUl.focus();else this.$selectableUl.focus()},"activeMouse":function($list){var that=this;$("body").on("mouseenter",that.elemsSelector,function(){$(this).parents(".ms-container").find(that.elemsSelector).removeClass("ms-hover");$(this).addClass("ms-hover")});
$("body").on("mouseleave",that.elemsSelector,function(){$(this).parents(".ms-container").find(that.elemsSelector).removeClass("ms-hover")})},"refresh":function(){this.destroy();this.$element.multiSelect(this.options)},"destroy":function(){$("#ms-"+this.$element.attr("id")).remove();this.$element.css("position","").css("left","");this.$element.removeData("multiselect")},"select":function(value,method){if(typeof value==="string")value=[value];var that=this,ms=this.$element,msIds=$.map(value,function(val){return that.sanitize(val)}),
selectables=this.$selectableUl.find("#"+msIds.join("-selectable, #")+"-selectable").filter(":not(."+that.options.disabledClass+")"),selections=this.$selectionUl.find("#"+msIds.join("-selection, #")+"-selection").filter(":not(."+that.options.disabledClass+")"),options=ms.find("option:not(:disabled)").filter(function(){return $.inArray(this.value,value)>-1});if(method==="init")selectables=this.$selectableUl.find("#"+msIds.join("-selectable, #")+"-selectable"),selections=this.$selectionUl.find("#"+msIds.join("-selection, #")+
"-selection");if(selectables.length>0){selectables.addClass("ms-selected").hide();selections.addClass("ms-selected").show();options.prop("selected",true);that.$container.find(that.elemsSelector).removeClass("ms-hover");var selectableOptgroups=that.$selectableUl.children(".ms-optgroup-container");if(selectableOptgroups.length>0){selectableOptgroups.each(function(){var selectablesLi=$(this).find(".ms-elem-selectable");if(selectablesLi.length===selectablesLi.filter(".ms-selected").length)$(this).find(".ms-optgroup-label").hide()});
var selectionOptgroups=that.$selectionUl.children(".ms-optgroup-container");selectionOptgroups.each(function(){var selectionsLi=$(this).find(".ms-elem-selection");if(selectionsLi.filter(".ms-selected").length>0)$(this).find(".ms-optgroup-label").show()})}else if(that.options.keepOrder&&method!=="init"){var selectionLiLast=that.$selectionUl.find(".ms-selected");if(selectionLiLast.length>1&&selectionLiLast.last().get(0)!=selections.get(0))selections.insertAfter(selectionLiLast.last())}if(method!=="init"){ms.trigger("change");
if(typeof that.options.afterSelect==="function")that.options.afterSelect.call(this,value)}}},"deselect":function(value){if(typeof value==="string")value=[value];var that=this,ms=this.$element,msIds=$.map(value,function(val){return that.sanitize(val)}),selectables=this.$selectableUl.find("#"+msIds.join("-selectable, #")+"-selectable"),selections=this.$selectionUl.find("#"+msIds.join("-selection, #")+"-selection").filter(".ms-selected").filter(":not(."+that.options.disabledClass+")"),options=ms.find("option").filter(function(){return $.inArray(this.value,
value)>-1});if(selections.length>0){selectables.removeClass("ms-selected").show();selections.removeClass("ms-selected").hide();options.prop("selected",false);that.$container.find(that.elemsSelector).removeClass("ms-hover");var selectableOptgroups=that.$selectableUl.children(".ms-optgroup-container");if(selectableOptgroups.length>0){selectableOptgroups.each(function(){var selectablesLi=$(this).find(".ms-elem-selectable");if(selectablesLi.filter(":not(.ms-selected)").length>0)$(this).find(".ms-optgroup-label").show()});
var selectionOptgroups=that.$selectionUl.children(".ms-optgroup-container");selectionOptgroups.each(function(){var selectionsLi=$(this).find(".ms-elem-selection");if(selectionsLi.filter(".ms-selected").length===0)$(this).find(".ms-optgroup-label").hide()})}ms.trigger("change");if(typeof that.options.afterDeselect==="function")that.options.afterDeselect.call(this,value)}},"select_all":function(){var ms=this.$element,values=ms.val();ms.find('option:not(":disabled")').prop("selected",true);this.$selectableUl.find(".ms-elem-selectable").filter(":not(."+
